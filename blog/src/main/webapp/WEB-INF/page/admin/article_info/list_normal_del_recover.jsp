...
<table id="table" class="table table-hover" style="margin-left:40px; margin-right: auto;">
    <br />
    <button type="button" id="recover" class="btn btn-default" style="margin-left:40px; margin-right: auto;">
        <span class="glyphicon glyphicon-heart"></span> 恢复
    </button>

    <a href="${pageContext.request.contextPath}/article_info/listNormal.controller">
        <button type="button" class="btn btn-default">
            <span class="glyphicon glyphicon-ok"></span> 不想恢复了...
        </button>
    </a>
    ...
    <div>
        <ul class="pagination">
            ...
            <%--当前${pageInfo.pageNum}页,共${pageInfo.pages}页,总${pageInfo.total }条记录--%>
              ...
              -->
            <li>
                <a href="${pageContext.request.contextPath}/article_info/listNormal_out.controller?pageNumber=1">首页</a>
            </li>
            <!-- 如果还有前页就访问当前页码-1的页面， -->
            <c:if test="${pageInfo.hasPreviousPage}">
                <li>
                    <a href="${pageContext.request.contextPath}/article_info/listNormal_out.controller?pageNumber=${pageInfo.pageNum-1}" aria-label="Previous">
            ...
            <c:if test="${page_Nums!=pageInfo.pageNum }">
                <li ><a href="${pageContext.request.contextPath}/article_info/listNormal_out.controller?pageNumber=${page_Nums}">${page_Nums}</a></li>
            </c:if>
            </c:forEach>
            </li>
            <!-- 如果还有后页就访问当前页码+1的页面， -->
            <c:if test="${pageInfo.hasNextPage}">
                <li>
                    <a href="${pageContext.request.contextPath}/article_info/listNormal_out.controller?pageNumber=${pageInfo.pageNum+1}" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
                </li>
            </c:if>
            <li><a href="${pageContext.request.contextPath}/article_info/listNormal_out.controller?pageNumber=${pageInfo.pages}">末页</a></li>
        </ul>
    </div>
</div>
</body>
<script>
    ...
    $("#recover").click(function () {
        ...
        if(idArr.length!=0){
            $.ajax({
                url:"${pageContext.request.contextPath}/article_info/recover.json",
                type:"post",
                dataType:"json",
                traditional:"true",
                data:{
